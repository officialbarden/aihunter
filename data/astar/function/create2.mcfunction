...
data modify entity @s Owner set from entity @n[type=snowball,tag=astar,tag=astar.cur] UUID
...
scoreboard players operation @s astar.g += @n[type=snowball,tag=astar,tag=astar.cur] astar.g
scoreboard players add @s astar.g 10
execute at @n[type=snowball,tag=astar,tag=astar.cur] if predicate astar:diagonal run scoreboard players add @s astar.g 4
