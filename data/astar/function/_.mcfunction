...
kill @e[type=snowball,tag=astar]
...
execute align xyz run summon snowball ~.5 ~.5 ~.5 {UUID:[I;-1320175556,-1270790790,-1995486549,-810104026],Tags:["astar"]}
execute at @s align xyz run summon snowball ~.5 ~.5 ~.5 {UUID:[I;468780849,-1207613367,-1699389816,-335253658],Tags:["astar"]}
execute at 1bf10731-b805-4849-9ab5-5e88ec046f66 align xyz if entity @n[predicate=astar:is_end,dx=0,dy=0,dz=0] run return run kill @e[type=snowball,tag=astar,dx=0,dy=0,dz=0,limit=2]
...
execute as 1bf10731-b805-4849-9ab5-5e88ec046f66 at @s summon snowball run function astar:create
# Loop till maxCommandChainLength or path is found
function astar:loop
# Mark the shortest path
execute unless entity @n[type=snowball,tag=astar,tag=astar.finalizer,tag=!astar.finished] at b14fb83c-b441-457a-890f-4aabcfb6cb26 as @n[type=snowball,tag=astar,tag=astar.path,tag=!astar.finished] at @s run function astar:finalize
execute at b14fb83c-b441-457a-890f-4aabcfb6cb26 as @n[type=snowball,tag=astar,tag=astar.finalizer,tag=!astar.finished] at @s run function astar:finalize
# Only leave the correct path
kill @e[type=snowball,tag=astar,tag=astar.path,tag=!astar.finished]
...
return run execute align xyz if entity @n[type=snowball,tag=astar,tag=astar.finalizer,dx=0,dy=0,dz=0]
